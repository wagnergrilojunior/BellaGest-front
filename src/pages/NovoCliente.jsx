...
import { Cliente, User } from "@/api/entities";
...
import { ArrowLeft, UserPlus, Upload, Calendar as CalendarIcon, MapPin, Info, Phone, ShieldCheck } from "lucide-react";
...
export default function NovoCliente() {
  ...
  const validate = () => {
    const newErrors = {};
    if (!dados.nome.trim()) newErrors.nome = "O nome completo é obrigatório.";
    if (!dados.telefone.trim()) newErrors.telefone = "O telefone é obrigatório.";
    if (!dados.endereco.cep.trim()) newErrors['endereco.cep'] = "O CEP é obrigatório.";
  ...
  const adicionarTag = () => {
    if (novaTag.trim() && !dados.tags.includes(novaTag.trim())) {
      setDados(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await Cliente.create(dadosParaEnviar);
      navigate(createPageUrl("Clientes"));
    } catch (error) {
      console.error('Erro ao criar cliente:', error);
      setErrors(prev => ({ ...prev, geral: "Erro ao criar cliente. Por favor, tente novamente." }));
    }
    setLoading(false);
  };
        ...
        <form onSubmit={handleSubmit}>
          <Tabs defaultValue="pessoal" className="w-full">
            <TabsList className="grid w-full grid-cols-1 md:grid-cols-4 mb-6">
              <TabsTrigger value="pessoal"><UserPlus className="w-4 h-4 mr-2" />Dados Pessoais</TabsTrigger>
            ...
            <TabsContent value="adicionais">
               <div className="space-y-6">
                ...
                <Card className={`glass-effect border-0 shadow-lg ${errors.consentimento_lgpd ? 'border border-red-500' : ''}`}>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2"><ShieldCheck className="w-5 h-5 mr-1" /> LGPD</CardTitle>
                  </CardHeader>
          ...
          {errors.geral && <p className="text-sm text-red-600 text-center mt-4">{errors.geral}</p>}
